...
setopt prompt_subst
PROMPT='%{$reset_color%}$(symbol)%{$reset_color%} %{$reset_color%}%n%{$reset_color%}%{$reset_color%}@%m%{$reset_color%} %~ %{$reset_color%}${vcs_info_msg_0_}%{$reset_color%}%(?,,%{${fg_bold[white]}%}[%?]%{$reset_color%} )%{$reset_color%}%#%{$reset_color%} '
